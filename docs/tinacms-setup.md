...
- **Images are referenced by filename, not uploaded through Tina (yet).** Fields like a
  product `image` or team `photo` store a filename (e.g. `coffee.jpg`); the site adds the
  folder in code (`/assets/products/coffee.jpg`). To add a *new* image today: add the file
  under `public/assets/...` (via the repo) and type its filename in the field.
  - *Optional enhancement:* switch these to Tina `image` fields with the media manager
    (Tina media is already pointed at `public/assets`) so editors can upload images from
    the admin. This requires changing the three render spots to use the stored path
    directly and migrating the existing filename values — a small, well-scoped follow-up.
...
- Editors can't add or delete whole files (create/delete is disabled); they edit the
  fields within each existing section. They *can* add/remove items within a list (e.g. a
